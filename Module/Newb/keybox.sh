...
REMOTE_URL="https://raw.githubusercontent.com/ahnanhaquemahi/Newbkey/main/conf"
...
# Detailed log
log_message() {
    echo "$(date +'%Y-%m-%d %H:%M:%S') [NEWB_KEYBOX] $1"
}
...
if [ ! -d "$DEPENDENCY_MODULE" ]; then
  ...
  log_message "Please install Tricky Store before using Newb Keybox."
  exit 1
fi
